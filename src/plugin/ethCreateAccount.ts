...
export class EthereumCreateAccount implements Interfaces.CreateAccount {
  private _accountName: EthereumAddress

  private _chainState: EthereumChainState

  private _multisigPlugin: PluginInterfaces.MultisigPlugin
  ...
  constructor(
    ...
    multisigPlugin?: PluginInterfaces.MultisigPlugin,
  ) {
  ...
  get multisigPlugin(): PluginInterfaces.MultisigPlugin {
  ...
  private assertHasMultisigPlugin() {
    if (!this.multisigPlugin) {
      Errors.throwNewError('EthereumCreateAccount error - multisig plugin is missing (required for multisigOptions)')
    }
  }
  ...
  private assertMultisigPluginIsInitialized() {
    ...
    if (!this.multisigPlugin?.isInitialized) {
      Errors.throwNewError('EthereumCreateAccount error - multisig plugin is not initialized')
    }
  }
}
