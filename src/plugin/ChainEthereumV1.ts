...
import { EthereumSignString } from './ethSignString'
...
class Plugin implements Interfaces.Chain {
  ...
  public get plugins(): Interfaces.ChainJsPlugin[] {
  ...
  private newSignString = async (data: any, options?: any): Promise<EthereumSignString> => {
    // this.assertIsConnected()
    const transaction = new EthereumSignString(data, options)
    return transaction
  }
  ...
  ): Promise<{ status: Models.TransactionStatus; transaction: Transaction }> {
  ...
  public toEntityName = (value: string): Models.ChainEntityName => {
    return toEthereumEntityName(value) as Models.ChainEntityName
  }
  ...
  public toDate = (value: string | Date | EthereumDate): Models.ChainDate => {
    return toEthereumDate(value) as Models.ChainDate
  }
  ...
  public mapChainError = (error: Error): Errors.ChainError => {
  ...
  public async installPlugin(plugin: Interfaces.ChainJsPlugin, options?: Interfaces.ChainJsPluginOptions) {
  ...
  public get multisigPlugin(): EthereumMultisigPlugin {
    return this._plugins?.find(plugin => plugin?.type === Interfaces.PluginType.MultiSig)
  }

  /** rules to check tha plugin is well-formed and supported */
  private assertValidPlugin(plugin: Interfaces.ChainJsPlugin) {
