...
export class GnosisSafeMultisigPluginCreateAccount implements EthereumMultisigPluginCreateAccount {
  ...
  public async getMultisigAddressFromOptions() {
    ...
    if (!Helpers.isNullOrEmpty(owners) || !Helpers.isNullOrEmpty(threshold) || !Helpers.isNullOrEmpty(nonce)) {
      ...
      if (this.multisigAddress && calculatedAddress !== this.multisigAddress) {
        Errors.throwNewError('multisigAddress and multisigOptions do not match!')
      }
    } else if (!this.multisigAddress) {
      Errors.throwNewError('must provide either multisigAddress or multisigOptions (to calculate multisig address)')
    }
  ...
  get transactionAction(): EthereumTransactionAction {
    if (!this._createAccountTransactionAction)
      Errors.throwNewError('createAccountTransactionAction is missing. Make sure you init() with createAccountOptions')
  ...
  public async getTransactionFromOptions() {
    ...
    // if ANY multisigOptions is provided, then calculate the multisig address
    if (!Helpers.isNullOrEmpty(owners) || !Helpers.isNullOrEmpty(threshold) || !Helpers.isNullOrEmpty(saltNonce)) {
