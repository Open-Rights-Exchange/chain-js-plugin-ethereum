...
export class GnosisSafeMultisigPluginTransaction implements EthereumMultisigPluginTransaction {
  ...
  public get signBuffer(): Buffer {
    return Helpers.toBuffer(this._transactionHash, 'hex')
  }
  ...
  private assertSignatureOwnerValidAndUnique(signatures: GnosisSafeSignature[]): GnosisSafeSignature[] {
    ...
    signatures.forEach(sigOrString => {
      ...
      if (!lowercaseOwners.includes(signature.signer.toLowerCase())) {
        Errors.throwNewError(`Signature data:${signature.data} does not belong to any of the multisig owner addresses`)
      }
      signaturesArray.push(signature)
    })
    const uniqueSignatures = Helpers.getUniqueValues<GnosisSafeSignature>(Array.from(signaturesArray))
  ...
  private assertValidOptions(options: EthereumGnosisMultisigTransactionOptions) {
    ...
    if (Helpers.isNullOrEmpty(multisigAddress)) {
      Errors.throwNewError('If plugin is initialized for transaction, multisigAddress field must exists')
    }
  }
  ...
  private async setParentTransactionIfReady(): Promise<void> {
    if (!Helpers.isNullOrEmpty(this.missingSignatures)) return
  ...
  public validate(): Promise<void> {
    if (!this.rawTransaction) {
      Errors.throwNewError('safeTransaction is missing. Call prepareToBeSigned()')
    }
    return null
  }
