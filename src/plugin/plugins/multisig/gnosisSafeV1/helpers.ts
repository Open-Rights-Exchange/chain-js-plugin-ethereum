...
export function isValidGnosisSignature(value: GnosisSafeSignature) {
  ...
  if (typeof value === 'string') {
    signature = Helpers.tryParseJSON(value) || {}
  } else {
    signature = value
  }
...
export function assertValidGnosisSignature(signature: GnosisSafeSignature) {
  if (!isValidGnosisSignature(signature)) {
    Errors.throwNewError(`Not a valid Gnosis signature : ${signature}`, 'signature_invalid')
  }
}
...
export function toGnosisSignature(value: string | GnosisSafeSignature): GnosisSafeSignature {
  const signature = (typeof value === 'string' ? Helpers.tryParseJSON(value) : value) as GnosisSafeSignature
...
export function toStringifiedEthereumSignatureFromGnosisSignature(gnosisSignature: GnosisSafeSignature) {
  if (Helpers.isAString(gnosisSignature)) return gnosisSignature as EthereumSignature
...
export function assertMultisigOptionsForProxyArePresent(multisigOptions: EthereumGnosisMultisigCreateAccountOptions) {
  if (
    Helpers.isNullOrEmpty(multisigOptions?.owners) ||
    Helpers.isNullOrEmpty(multisigOptions?.threshold) ||
    isNullOrEmptyEthereumValue(multisigOptions?.gnosisSafeMaster) ||
    isNullOrEmptyEthereumValue(multisigOptions?.proxyFactory) ||
    Helpers.isNullOrEmpty(multisigOptions?.saltNonce)
  ) {
...
export async function calculateProxyAddress(
  ...
  try {
    ...
    Errors.throwNewError('Invalid create options. Try increasing saltNonce')
  }
  return address
}
...
export async function transactionToSafeTx(
  ...
  let safeTxData
  if (isNullOrEmptyEthereumValue(data) && !Helpers.isNullOrEmpty(contract)) {
...
export function populatedToRawEthereumTransaction(populatedTrx: PopulatedTransaction): EthereumRawTransactionAction {
  ...
  const transactionObject = {
    from: Helpers.nullifyIfEmpty(toEthBuffer(from)),
    to: Helpers.nullifyIfEmpty(toEthBuffer(to)),
    value: Helpers.nullifyIfEmpty(toEthBuffer(valueHex)),
    data: Helpers.nullifyIfEmpty(toEthBuffer(data)),
    gasPrice: Helpers.nullifyIfEmpty(toEthBuffer(gasPriceHex)),
    gasLimit: Helpers.nullifyIfEmpty(toEthBuffer(gasLimitHex)),
    nonce: Helpers.nullifyIfEmpty(toEthBuffer(nonce)),
  }
  Helpers.removeEmptyValuesInJsonObject(transactionObject)
