...
export class EthereumChainState implements Interfaces.ChainState {
  ...
  public async getBlock(blockNumber: EthereumBlockNumber): Promise<BlockTransactionString> {
    try {
      ...
      if (Helpers.isNullOrEmpty(block)) {
  ...
  public async getTransactionCount(address: EthereumAddress, defaultBlock: EthereumBlockNumber): Promise<number> {
    try {
      return await this._web3.eth.getTransactionCount(Helpers.ensureHexPrefix(address), defaultBlock)
    } catch (error) {
  ...
  public async fetchBalance(
    ...
    if (Helpers.isNullOrEmpty(tokenAddress)) {
    ...
    if (Helpers.isNullOrEmpty(erc20Contract)) {
  ...
  public async getErc20TokenBalance(
    ...
    if (balance && decimals) {
      balanceString = Helpers.bigNumberToString(balance, decimals)
    }
  ...
  public isMethodCallable(contract: Contract, methodName: string): boolean {
    if (Helpers.objectHasProperty(contract, methodName)) {
      return true
    }
    return false
  }
  ...
  */
  ...
  public async fetchTransaction(
    ...
    if (!transaction) throw new Errors.ChainError(Models.ChainErrorType.TxNotFoundOnChain, 'Transaction Not Found')
    const status = transaction.blockNumber ? Models.TransactionStatus.Executed : Models.TransactionStatus.Pending
